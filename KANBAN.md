...
- [ ] BUG: WYSIWYG Frame Designer live preview freezes after saving a per-card frame | Priority: P1 | Started: 2026-07-03 | Owner: drew-valentine
  ...
  - Fix (in progress): Make the live designer settings authoritative for the preview endpoints (same treatment `text_overrides` already received), so live changes drive the preview even when `frame_overrides` are already persisted for the card.
  - Acceptance criteria:
