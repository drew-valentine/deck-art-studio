...
5. The images themselves also condition every render (**FLUX Redux**). The **Reference strength** slider sets how strongly: *Palette* borrows only the colors, *Subtle* (default) carries the reference's medium and palette while each card keeps its own subject, and *Balanced* brings in the reference's full idiom — the ceiling, because references with prominent characters start to put those characters on your cards above it. By default the **first** inspiration image is the reference (put your best style example first); every additional reference also brings its *content*, so more isn't better. **Tip:** references without prominent characters (scenery, patterns, texture) transfer style cleanly at higher settings; character-heavy references are best kept at Subtle.
