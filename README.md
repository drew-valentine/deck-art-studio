...
| **End-of-batch render inspection** | The scene checklist judges the prompt; nothing judged the picture. A batch now ends with an inspection job: the vision model checks every new render for anatomy and duplication defects (extra or missing limbs and fingers, doubled heads, duplicated subjects, malformed hands or faces), stray text, signatures and watermarks, and a missing subject. It runs once per batch because the vision model cannot sit beside FLUX in 18 GB. Verdicts are stamped into each render's metadata; defective cards are re-rolled once and inspected again. With two takes per card, the inspection also compares the takes and keeps the cleaner one as current, archiving the other. |
