...
5. The images themselves also condition every render (**FLUX Redux**). The **Reference strength** slider sets how strongly: *Palette* borrows only the colors, *Subtle* (default) carries the reference's medium and palette while each card keeps its own subject, *Balanced* and *Strong* bring in more of the reference's idiom — but references with prominent characters start to put those characters on your cards — and *Clone* renders a variation of the reference. Up to 4 references are used per render. **Tip:** references without prominent characters (scenery, patterns, texture) transfer style cleanly at higher settings; character-heavy references are best kept at Subtle.
