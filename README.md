...
5. The images themselves also condition every render (**FLUX Redux**). The **Reference strength** slider sets how strongly: *Palette* borrows only the colors, *Balanced* (default) carries the reference's medium and idiom while each card keeps its own scene, *Clone* renders a variation of the reference. Up to 4 references are used per render.
