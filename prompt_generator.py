...
def generate_subject_with_ai(card: dict, openai_client=None, backend: str = 'openai',
    ...
    system_msg = (
        ...
        "itself — name it and state its physical form within the first ten words "
        "(e.g. 'Zndrsplt, a one-eyed homunculus creature, crouches...'), and only "
        "THEN place it in the scene. NEVER open with the setting, weather, or "
        "atmosphere ('In the heart of the swirling mist...') — the image model "
        "paints whatever comes first, and setting-first openings produce "
        "subjectless art. "
